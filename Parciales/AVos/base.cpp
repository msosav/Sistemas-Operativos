...
int main(int argc, char *argv[]) {
  ...
  if (string(argv[2]) == "Rotar") {
    if (argc != 4) {
      cerr << "Uso: " << argv[0] << " <nombreArchivo>" << argv[1] << " <Operacion (Rotar)>" << argv[2] << " <Medida (Grados)>" endl;
      return 1;
    }
    auto start_time = std::chrono::high_resolution_clock::now();
    matrizImagenSalida = rotarImagen(matrizImagenEntrada, atoi(argv[3]), headerImagenSalida);
    ...
    auto start_time = std::chrono::high_resolution_clock::now();
    matrizImagenSalida = escalarImagen(matrizImagenEntrada, atoi(argv[3]), atoi(argv[4]), headerImagenSalida);
    auto end_time = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
    cout << "Tiempo de ejecucion: " << duration << " ms" << endl
    ...
    cerr << "Operacion no valida" << endl;
      ...
      << argv[3] << " <Height (si se va a escalar)>" endl;
    return 1;
  }
  return 0;
}
